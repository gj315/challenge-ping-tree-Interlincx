...
var targetController = require('./target/targetController')
...
// POST: Add a new target, GET: Retrieve all targets
router.set('/api/targets', {
  POST: targetController.addTarget,
  GET: targetController.getAllTargets
})

// POST: Update a target by ID, GET: Retrieve a target by ID
router.set('/api/target/:id', {
  POST: targetController.updateTargetById,
  GET: targetController.getTargetById
})

// POST: Determines decision based on visitor's info
router.set('/route', { POST: targetController.determineDecision })
