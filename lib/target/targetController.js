...
/**
 * Determines decision based on visitor's info, target criteria, and returns either the highest value target URL or a rejection.
 *
 * @param {Object} req - The HTTP request object.
 * @param {Object} res - The HTTP response object.
 * @param {Object} options - Optional parameters for the request.
 * @param {Function} callback - Callback function to handle the response.
 */
function determineDecision (req, res, options, callback) {
  body(req, res, function (err, bodyResponse) {
    if (err) return utils.badRequest(req, res, err)
    utils.validateDecisionReqBody(bodyResponse, function (err, reqBody) {
      if (err) return utils.badRequest(req, res, err)
      targetServices.getDetermineDecision({ reqBody, req, res, options, callback })
    })
  })
}
